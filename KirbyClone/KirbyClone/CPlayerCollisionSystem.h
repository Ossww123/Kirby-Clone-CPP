...
public:
    ...
    // === Ground 상태 업데이트 ===
    void UpdateGroundState();           // 매 프레임 Ground 상태 체크

private:
    // === Ground 체크 헬퍼 함수들 ===
    bool IsPlayerOnGround() const;      // 플레이어가 실제로 땅 위에 있는지 체크
    bool CheckGroundBelow() const;      // 플레이어 발 아래 타일이 있는지 체크
    CTile* FindSupportingTile() const;  // 현재 지지하고 있는 타일 찾기

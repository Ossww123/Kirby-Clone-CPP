...
CPlayerStateMachine::CPlayerStateMachine(CPlayer* _pOwner)
    ...
    , m_bWasGrounded(true)
    , m_fSlideTimer(0.0f)
    , m_fSlideDuration(0.8f)       // 0.8초 동안 슬라이드
    , m_fSlideDistance(256.f)      // 4타일 거리 (64*4)
    , m_fSlideSpeed(320.f)         // 슬라이드 속도
...
void CPlayerStateMachine::InitiateSlide()
{
    ...
    {
        m_iSlideDirection = 1; // 기본값: 오른쪽
    }

    // 슬라이드 속도 적용
    m_pRigidBody->SetVelocityX(m_fSlideSpeed * m_iSlideDirection);
}
...
void CPlayerStateMachine::UpdateSlideState()
{
    ...
    // 지면에서 벗어났는지 체크
    if (m_bSlideGroundCheck && !m_pRigidBody->IsGround())
    {
        HandleSlideToFall();
        return;
    }
...
void CPlayerStateMachine::UpdateSlideMovement()
{
    ...
    {
        // 슬라이드 속도 점진적 감소 (처음에는 빠르게, 나중에는 천천히)
        float speedMultiplier = 1.0f - (slideProgress * slideProgress); // 제곱 감소
        ...
        m_pRigidBody->SetVelocityX(currentSlideSpeed * m_iSlideDirection);
    }
}
...
void CPlayerStateMachine::CheckSlideCompletion()
{
    ...
    if (currentSpeedX < 50.f) // 최소 속도 이하로 떨어지면
    {
        shouldEndSlide = true;
    }

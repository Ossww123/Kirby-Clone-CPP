...
public:
    enum class INPUT_TYPE : uint32_t
    {
        ...
        JUMP = 1 << 4 ,             // Z 키 (점프)
        ACTION = 1 << 5 ,           // X 키 (카피능력/슬라이드/공격)
        DROP_ABILITY = 1 << 6 ,     // BACKSPACE 키 (능력 버리기)

        // === TAP, HOLD, AWAY 이벤트 ===
        JUMP_TAP = 1 << 8 ,         // Z 키 TAP (점프 시작)
        JUMP_HOLD = 1 << 9 ,        // Z 키 HOLD (점프 지속 또는 호버)
        JUMP_AWAY = 1 << 10 ,       // Z 키 AWAY (점프 종료)
...
private:
    ...
    // Movement 상태를 추적하여에 추가 변수들
    bool m_bLeftDecelerating;           // 왼쪽 키 놓은 후
    bool m_bRightDecelerating;          // 오른쪽 키 놓은 후
    ...
    bool m_bJumpCurrentlyPressed;       // 현재 Z 키 상태 (점프 홀드 추적용)
